...
namespace ariel
{
    ...
    Player::Player(const Player &other)
        : name(other.name), points(other.points), resources(other.resources), color(other.color), numerOfKnightCard(other.numerOfKnightCard)
    {
    ...
    void Player::buildSettlement(Vertex &location)
    {
        ...
        if (location.getNumber() < 0 || location.getNumber() > 53)
        {
            ...
            resources[Resource::Wool] >= 1 && resources[Resource::Oats] >= 1)
        {
            ...
            {
                throw std::runtime_error("Cannot build settlement. Another player's settlement or city is too close.");
                // return;
            }
            ...
            if (location.getCity() != NULL || location.getSettlement() != NULL)
            {
                tooClose = true;
            }
        ...
        {
            throw std::runtime_error("Not enough resources to build a settlement.");
        }
    }
    ...
    void Player::buildCity(Vertex &location)
    {
        ...
        if (resources[Resource::Iron] >= 3 && resources[Resource::Oats] >= 2)
        {
            ...
            {
                throw std::runtime_error("No settlement found at this location to upgrade to a city.");
            }
        }
        else
        {
    ...
    void Player::buildRoad(Vertex &start, Vertex &end, Board *board)
    {
        ...
        if (resources[Resource::Brick] >= 1 && resources[Resource::Wood] >= 1)
        {
            ...
            if (!roadExists)
            {
                ...
                {
                    throw std::runtime_error("Cannot build road: Must be connected to a building or another road of the player.");
                }
            }
            else
            {
                throw std::runtime_error("Cannot build road: A road already exists between these vertices.");
            }
        }
        else
        {
            throw std::runtime_error("Not enough resources to build a road.");
        }
    }
    ...
    void Player::tradeResources(Player &otherPlayer, Resource offeredResource, Resource desiredResource, int quantity)
    {
        if (this == &otherPlayer)
        {
            // Handle the case where the players are the same (optional)
            throw std::invalid_argument("Cannot trade resources with yourself.");
        }
        ...
        {
            throw std::runtime_error("Trade failed. Insufficient resources or disagreement.");
        }
    }
